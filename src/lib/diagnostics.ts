...
const SENSITIVE_KEY =
  /(?:authorization|cookie|credential|password|private[-_ ]?key|secret|signature|token|api[-_ ]?key|query|variables|request[-_ ]?body|response[-_ ]?body|body|headers?|prompt|content|text)/iu;
...
const safeDiagnosticMessages = new WeakMap<object, string>();
...
export interface DiagnosticLoggerOptions {
  ...
  readonly write?: DiagnosticWriter;
  readonly now?: () => Date;
  readonly id?: () => string;
}
...
export interface DiagnosticSpan {
  ...
  skipped(details?: unknown): void;
  cancelled(error?: unknown, details?: unknown): void;
}
...
export function registerSafeDiagnosticError(
  error: unknown,
  message: string,
  primitiveMessages?: Map<unknown, string>,
): void {
  if (isObjectLike(error)) safeDiagnosticMessages.set(error, message);
  else primitiveMessages?.set(error, message);
}
...
function truncate(value: string, maxLength = MAX_DIAGNOSTIC_STRING_LENGTH): string {
...
function stripEmbeddedUrlQueries(value: string): string {
  return value.replace(/https?:\/\/[^\s"'<>]+/giu, (candidate) => stripUrlQuery(candidate));
}

function safeKey(key: string, secrets: readonly string[]): string {
...
function project(
  ...
  if (key !== undefined && isSensitiveKey(key)) return "[OMITTED]";
      ...
      if (rawKey === "response_headers") {
        ...
        for (const [header, headerValue] of Object.entries(child)) {
          if (SAFE_RESPONSE_HEADERS.has(header.toLowerCase()))
            headers[header.toLowerCase()] = project(
              headerValue,
              secrets,
              state,
              undefined,
...
function errorRecordValue(
  error: unknown,
  secrets: readonly string[],
  depth: number,
  primitiveMessages?: ReadonlyMap<unknown, string>,
): unknown {
  if (depth > MAX_DIAGNOSTIC_CAUSE_DEPTH) return "[CAUSE_DEPTH_EXCEEDED]";
  ...
  if (registeredMessage !== undefined) {
    ...
    return project(
      { name: error instanceof Error ? error.name : typeof error, message: registeredMessage },
      secrets,
      state,
      undefined,
      0,
  ...
  if (error instanceof Error) {
    ...
    const stack =
      typeof error.stack !== "string" || typeof diagnosticMessage === "string"
        ? undefined
        : error.stack;
    ...
    for (const [key, value] of Object.entries(error)) {
      if (safeErrorKeys.has(key)) record[key] = value;
    }
  ...
  return project(
    {
      name: typeof error,
      message:
        primitiveMessages?.get(error) ?? (error === undefined ? "undefined" : "[UNSERIALIZABLE]"),
    },
    secrets,
    state,
    undefined,
...
function serialized(value: unknown, secrets: readonly string[]): string {
  ...
  if (result.length <= MAX_DIAGNOSTIC_LINE_LENGTH) return result;
  if (isRecord(projected)) {
    const { details, ...base } = projected;
    const retainedDetails: Record<string, unknown> = {
      truncated: true,
    ...
    if (isRecord(details)) {
      for (const key of [
        ...
        if (Object.hasOwn(details, key)) retainedDetails[key] = details[key];
      }
      if (Object.hasOwn(details, "graphql_errors")) retainedDetails.graphql_errors = "[TRUNCATED]";
      if (isRecord(details.error)) {
        ...
        for (const key of [
          ...
          if (Object.hasOwn(details.error, key)) error[key] = details.error[key];
        }
        if (Object.hasOwn(details.error, "graphqlErrors")) error.graphqlErrors = "[TRUNCATED]";
        if (Object.keys(error).length > 0) retainedDetails.error = error;
      }
    }
...
export class DiagnosticLogger {
  ...
  private readonly primitiveDiagnosticMessages = new Map<unknown, string>();
  ...
  addSecrets(secrets: readonly string[]): void {
    for (const secret of secrets) if (secret.length > 0) this.secrets.add(secret);
  }

  registerSafeDiagnosticError(error: unknown, message: string): void {
  ...
  start(descriptor: DiagnosticDescriptor, details?: unknown): DiagnosticSpan {
  ...
  async withSpan<T>(
    ...
    try {
      ...
      if (isCancellation(error)) span.cancelled(error);
