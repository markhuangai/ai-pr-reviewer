...
- The action writes the full secret-redacted system prompt, `/goal`, review and repair user prompts, assistant text, and validation or tool errors to the GitHub Actions log. Long messages use numbered chunks so the redacted content remains reconstructable. Session initialization and completion, goal iterations, turn results, repairs, and automatic compaction attempts and boundaries are logged explicitly. Successful tool inputs, diff pages, and ordinary successful results remain bounded previews; hidden model reasoning is not logged.
