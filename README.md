...
- The fixed Git diff uses merge-base attributes so pull-request changes cannot hide text as binary. Pages are bounded by bytes and tool results are capped before delivery; a goal is not required to consume a monolithic diff or every conversation body. Bash and other write or execution tools remain unavailable.
