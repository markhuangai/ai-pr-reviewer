...
- A goal must submit a schema-validated result and an evidence-backed assessment through the internal `submit_review` MCP tool. Host tool hooks issue evidence references; each changed path must be marked reviewed, not applicable with a reason, or incomplete. Reviewed and not-applicable paths require completed repository evidence at the appropriate revision: head or an applicable diff for additions/modifications, base or diff for deletions and old rename paths. Earlier page references become usable only after their own complete query has been delivered; unrelated queries are never combined. Native whole-file reads use returned bounds and truncation metadata, so a sufficiently large requested limit does not make a complete read partial. A supported finding must link to a supported candidate whose completed repository evidence covers its affected path. An incomplete result keeps valid findings, is published only as a comment, and cannot approve a pull request. One initial submission plus five same-session corrections are allowed, including SDK schema rejections and empty terminal turns; reads and compaction do not reset the allowance. On exhaustion, only independently valid findings from the latest candidate survive, with honest coverage gaps and an incomplete status. A malformed replacement cannot revive older findings. No extra model turn is added for finalization. Goals without safe retained findings fail.
