...
- Findings are sorted by severity, deduplicated across goals, and limited to 25 inline comments. Each comment uses a severity marker, a short impact statement, and a short fix. A finding may also include GitHub's native apply-suggestion block when its complete replacement fits one verified contiguous diff range. A location that is not an added line in the pull request diff is moved into the review body without the suggestion.
