...
- Review analysis uses the captured base, head, files, and conversation. When interactive lifecycle candidates exist, the action reloads REST reviews and comments after reconciliation before duplicate detection and review generation; each lifecycle mutation also rereads GraphQL review-thread state immediately before it runs. A workflow consumer can use `concurrency.cancel-in-progress` when it wants newer events to supersede an active run.
...
- After the current review is posted, an older action finding review is minimized only when every inline thread in that review is resolved and the review has no body-only findings. Lifecycle reconciliation uses the GitHub GraphQL API and is skipped entirely when `interact-with-pr: false`.
