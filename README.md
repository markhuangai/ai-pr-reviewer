...
- Each goal must sweep every changed path through completed full or selected diffs, or equivalent complete source at the appropriate revision, then investigate relevant functions and callers. The runtime tracks inspection; the model does not reconstruct a coverage table. Head reads or applicable diffs cover additions/modifications; base reads or diffs cover deletions and old rename paths. Earlier page references become usable only after their own complete query has been delivered. A successful bounded native read supports a finding inside its verified returned range, but only verified whole-file reads can complete source inspection. Failed, truncated, missing, or contradictory delivery cannot certify inspection. Binary and metadata-only changes can be swept through their diff metadata without claiming binary-content review.
- The internal `submit_review` tool accepts exactly `summary`, `findings`, and `limitations`. Findings carry `evidenceRefs`, a `countercheck`, and `counterevidenceRefs` alongside their existing public fields. Proof stays private. Limitations contain `paths` and `reason`; empty paths means a goal-wide limit. Empty limitations declares finished investigation, but cannot override missing runtime inspection. Explicit incomplete submissions keep validated findings and cannot approve a pull request. Unfinished optional reads alone do not block completion.
- One initial submission plus five same-session corrections are allowed, including SDK schema rejections and empty terminal turns; reads and compaction do not reset the allowance. On exhaustion, only independently valid findings from the latest allowed submission survive, with explicit limitations and an incomplete status. Withdrawn findings cannot return, and a malformed replacement cannot revive older findings. No extra model turn is added for finalization. Goals without safe retained findings fail. Acceptance retains the existing bounded finalization and required MCP checks.
